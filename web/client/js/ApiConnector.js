...
function GpsHandle(){
	...
	GpsHandle.prototype.initGps = function initGps(){
    	db = Lawnchair({name : 'db'}, function(store) {
        	...
        	setInterval(function() {window.GPS.upload(store)},3000);//upload locations to the server every 30 seconds
    	});
	}
...
function MapHandle(){
	...
	MapHandle.prototype.initMap = function initMap(){
	    ...
	    var mapOptions = {
		  ...
		  google.maps.event.addListener(map, 'mousedown', window.MAP.markerSelectDown);
		  google.maps.event.addListener(map, 'mouseup', window.MAP.markerSelectUp);
	}
	...
	MapHandle.prototype.markerSelectUp = function markerSelectUp(event){
	    markerEvent = event;
	...
	MapHandle.prototype.markerSelectDown = function markerSelectDown(event){
	    markerEvent = event;
