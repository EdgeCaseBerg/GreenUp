...
$urlArr = explode("url=", $actual_link);
$goodUrl = $urlArr[1];
error_log("-----url ----");
error_log($goodUrl);
$url = $goodUrl;
...
//    error_log("fell into put");
  ...
  // messing with this to get PUTs to proxy
  // $resultsArr = explode('\n', $contents);
  $resultsArr = preg_split("/\\r\\n|\\r|\\n/", $contents);
  for($ii=0; $ii<sizeof($resultsArr); $ii++){
    if (strpos($resultsArr[$ii],"{") !== false){
      $contents = $resultsArr[$ii];
      break;
    }
  }
  // $contents = $resultsArr[8];
  // error_log($contents);
